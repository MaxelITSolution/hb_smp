	...
	<section class="datagrid-panel">
		...
		<section class="tile">
			<div class="tile-widget">
				<div class="row">
					<div class="col-sm-5">
						<button class="btn btn-sm btn-default" onclick="main_routes('create', '')"><i class="fa fa-plus"></i> New User</button>
					</div>
...
<script type="text/javascript">
	var datagrid = $("#datagrid").datagrid({
		url						: "<?php echo site_url('app/user/data') ?>",
		primaryField			: 'id', 
		rowNumber				: true,
		itemsPerPage			: 10,
		...
		columns					: [
        	{field: 'name', title: 'Nama Lengkap', editable: true, sortable: true, width: 150, align: 'left', search: true},
        	{field: 'phone_number', title: 'No Telepon', editable: true, sortable: true, width: 120, align: 'left', search: true},
	...
	function create_update_form(rowIndex) {
		...
		$.post("<?php echo site_url('app/user/form') ?>", {index : rowIndex}).done(function(data) {
	...
	function delete_action(rowIndex) {
		swal({   
			...
			$.post("<?php echo site_url('app/user/delete') ?>", {id : row['id']}).done(function(data) {
				datagrid.reload();
	        });
		});
	}
